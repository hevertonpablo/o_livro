...
### Direção literária (em `docs/`)
- [`docs/escopo.md`](docs/escopo.md) — brainstorm de origem (5.714 linhas).
- [`docs/prd-literario.md`](docs/prd-literario.md) — consolidação detalhada.
- [`docs/prd-direcao-literaria.md`](docs/prd-direcao-literaria.md) — **direção oficial** (limpa, decisória).
...
4. [`books/biblia-continuidade-cenarios.md`](books/biblia-continuidade-cenarios.md) — lugares-personagem.
5. [`books/biblia-continuidade-clima.md`](books/biblia-continuidade-clima.md) — estações e estados climáticos.
...
- [`books/cenarios/`](books/cenarios/) — 12 fichas + README.
- [`books/clima/`](books/clima/) — 4 estações + 6 estados + README.
- [`books/lugares/`](books/lugares/) — 10 tipos + README.
- [`books/objetos/`](books/objetos/) — fichas por tier + README.
- [`books/horarios/`](books/horarios/) — 7 períodos + horários canônicos + README.

### Estrutura operacional (raiz)
- [`README.md`](README.md) — entrada do repositório.
- [`AGENTS.md`](AGENTS.md) — comportamento obrigatório de agents.
- [`RULES.md`](RULES.md) — leis invioláveis.
- [`CONTEXT_BRIEF.md`](CONTEXT_BRIEF.md) — resumo curto.
- [`PROJECT_STATUS.md`](PROJECT_STATUS.md) — este arquivo.
- [`DECISIONS.md`](DECISIONS.md) — decisões canônicas.
- [`CHANGELOG.md`](CHANGELOG.md) — histórico global.
...
| Risco | Mitigação |
|---|---|
| Inconsistência entre bíblias ao escrever | Hub `biblia-continuidade.md` + hierarquia de fontes definida |
| Agent inventar canon | `AGENTS.md` + `DECISIONS.md` com status `pendente` para propostas |
| Decisões pendentes acumularem | Lista em `TASKS.md` + revisão periódica |
| Perda de contexto entre sessões de IA | `SESSION_LOG.md` + `CONTEXT_BRIEF.md` |
| Mudanças não rastreadas | `CHANGELOG.md` + changelog por livro |
